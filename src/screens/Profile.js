...
const Profile = () => {
  ...
  // Genel unmount temizliği: zamanlayıcıları temizle
  useEffect(() => {
    return () => {
  ...
  // Dashboard'tan openEdit paramıyla gelinirse otomatik düzenleme modunu aç
  useFocusEffect(
    useCallback(() => {
      try {
        const shouldOpenEdit = route?.params?.openEdit;
        if (shouldOpenEdit && !isEditMode) {
          toggleEditMode();
          // Paramı temizle ki geri dönünce tekrar tetiklenmesin
          navigation.setParams({ ...route.params, openEdit: false });
        }
      } catch {}
      return () => {};
    }, [route?.params?.openEdit, isEditMode, toggleEditMode, navigation])
  );
      ...
      >
        ...
        <View style={styles.profileMainContainer}>
          ...
          <TouchableOpacity style={styles.shareButton} onPress={handleShareProfile}>
            <Image source={require('../assets/images/icons/share.png')} style={styles.shareIcon} />
          </TouchableOpacity>
          ...
          <View style={styles.profileInfoContainer}>
            ...
            <View style={styles.badgesContainer}>
              ...
              <View style={styles.expertBadgeContainer}>
                <View style={styles.expertBadgesRow}>
                  <Animated.View 
                    style={[
                      styles.expertBadge, 
                  ...
                  >
                    ...
                    <Text style={[styles.expertBadgeText, { color: theme.colors.white }]}>
                      {isEditMode ? (tempUserData.expertTitle || 'Lüks Konut Uzmanı') : (currentUser.expertTitle || 'Lüks Konut Uzmanı')}
                    </Text>
                    {isEditMode && (
                      <TouchableOpacity 
                        style={[styles.editFieldButton, { right: -15, top: -5, position: 'absolute', zIndex: 1 }]}
                        onPress={() => startEditing('expertTitle')}
                      >
                        <Image source={require('../assets/images/icons/Edit_fill.png')} style={styles.editFieldIcon} />
                      </TouchableOpacity>
                    )}
                ...
                {/* Eski ve hatalı buton kaldırıldı */}
              </View>
            </View>
          </View>
        </View>

        {/* Sosyal Medya ve İletişim Badge'leri - Profil Resminin Altında */}
        ...
        >
          <View style={styles.statsContentWrapper}>
            <View style={styles.statItem}>
              ...
              <Text style={[styles.statLabel, { color: isDark ? theme.colors.white : theme.colors.text.primary }]}>Portföy</Text>
            </View>
            <View style={styles.statItem}>
              <Text style={[styles.statNumber, { color: theme.colors.error }]}>
                {userPortfolios.filter(p => p.isPublished).length}
              </Text>
              <Text style={[styles.statLabel, { color: isDark ? theme.colors.white : theme.colors.text.primary }]}>Yayında</Text>
            </View>
            <View style={styles.statItem}>
              <Text style={[styles.statNumber, { color: theme.colors.error }]}>
                {Math.floor((new Date() - new Date(currentUser.createdAt)) / (1000 * 60 * 60 * 24))}
              </Text>
              <Text style={[styles.statLabel, { color: isDark ? theme.colors.white : theme.colors.text.primary }]}>Gün</Text>
            </View>
          </View>
        </GlassmorphismView>
        ...
        {/* Portföylerim Bölümü - Tek Container (HATALI BÖLÜM YENİDEN YAZILDI) */}
        <GlassmorphismView
  ...
  return (
    <SafeAreaView edges={['left','right','bottom']} style={[styles.safeArea, { backgroundColor: 'transparent' }]}>
      ...
      >
        ...
        >
          <View style={[styles.header, { paddingTop: Math.max(insets.top, 0) + 12, paddingBottom: theme.spacing.lg }]}>
            ...
            <View style={styles.headerCenter}>
              ...
              >
                <Text style={[styles.headerTitle, { color: isDark ? theme.colors.white : theme.colors.navy }]}>
                  {isOwnProfile ? 'Profilim' : (currentUser?.name || 'Profil')}
                </Text>
              </TouchableOpacity>
            </View>
...
const createStyles = (theme, insets, isDark) => StyleSheet.create({
  ...
  contentContainer: {
    flexGrow: 1,
    paddingHorizontal: theme.spacing.lg,
    paddingTop: theme.spacing.md + 10,
  },
